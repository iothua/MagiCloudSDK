...
namespace MagiCloud.Interactive
{
    ...
    public class InteractiveSearch
    {
        ...
        /// 启动交互
        /// </summary>
        public void OnStartInteraction(GameObject target,bool isGrab,int handIndex,bool defaultInteraction = false)
        {
            var interactions = target.GetComponentsInChildren<DistanceInteraction>();

            if (interactions.Count() == 0) return;

            OnSend(target,isGrab,handIndex,interactions,defaultInteraction);

            OnPourAll(target,isGrab,handIndex,interactions,InteractionType.Pour);
            OnPourAll(target,isGrab,handIndex,interactions,InteractionType.All);
        ...
        void OnSend(GameObject target,bool isGrab,int handIndex,DistanceInteraction[] interactions,bool defaultInteraction = false)
        {
            //获取到所有的发送信息
        ...
        void OnPourAll(GameObject target,bool isGrab,int handIndex,DistanceInteraction[] interactions,
            InteractionType interactionType,bool defaultInteraction = false)
        {
            ...
            foreach (var interaction in interactionPours)
            {
                ...
                //Debug.Log("Pour IsGrab");
                interaction.IsGrab = true;
                interaction.HandIndex = handIndex;

                //if (MOperateManager.GetObjectGrab(0))
                //{
                //    Debug.Log(MOperateManager.GetObjectGrab(0).name + "TRUE");
                //}
                //else
                //{
                //    Debug.Log("TRUE");
