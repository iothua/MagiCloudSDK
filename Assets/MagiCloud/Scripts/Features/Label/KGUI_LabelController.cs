...
namespace MagiCloud.KGUI
{
    ...
    public class KGUI_LabelController :MonoBehaviour
    {
        ...
        public void SetJoinUI(Transform canvasNode)
        {
            Transform canvasTransform = canvasNode;
            if (canvasTransform!=null)
            {
        ...
        /// 初始化
        /// </summary>
        public void Initialize(Action initAction = null)
        {
            Transform canvasNode = GetCanvas();
            SetJoinUI(canvasNode);
        ...
        {
            //优先匹配KGUI
            Transform canvasNode = null;
            Canvas[] canvass = FindObjectsOfType<Canvas>();
            for (int i = 0; i < canvass.Length; i++)
                if (canvass[i].name=="KGUI_Canvas")
                {
                    canvasNode=canvass[i].transform;
                    return canvasNode;
                }
            if (canvasNode==null)
                canvasNode=FindObjectOfType<Canvas>().transform;
            return canvasNode;
        }
